...
* `--version`, `-V`: Print the OpenVMM build identity and exit. `-V` prints
  ...
  with tracked changes appends `.dirty`. A Git-free published source archive
  reports `MAJOR.MINOR.PATCH`. On Windows, the executable's `VERSIONINFO` uses
  the product version as `MAJOR.MINOR.PATCH.0`.
